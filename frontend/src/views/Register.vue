<template>
  <div class="auth-page">
    ...
    <div class="auth-card">
      ...
      <div v-if="success" class="alert alert-success">Account created! <router-link to="/login">Sign in here</router-link></div>
...
const register = async () => {
  ...
  if (form.value.name.trim().split(/\s+/).length < 2) { error.value = 'Please enter both first and last name.'; return }
  loading.value = true
  try {
    await authService.register({ userID: Number(form.value.userID), Name: form.value.name.trim(), role: form.value.role, password: form.value.password })
    success.value = true
  } catch (e) {
...
.auth-bg { position: absolute; inset: 0; background: radial-gradient(ellipse at 70% 50%, rgba(0,107,63,.3) 0%, transparent 60%), radial-gradient(ellipse at 20% 80%, rgba(245,168,0,.15) 0%, transparent 50%); }
