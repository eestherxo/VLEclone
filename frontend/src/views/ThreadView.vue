  ...
  <div class="thread-page">
    <div class="container py-6">
      ...
      <div class="forum-header card card-body">
        <h1>{{ forum.forumName || forum.title || forum.name || 'Forum' }}</h1>
      ...
      <div class="new-thread card card-body">
        ...
        <div class="form-group">
          <textarea v-model="newThread.content" class="form-control" rows="3" placeholder="Share your thoughts, questions or resources…"></textarea>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-sm text-muted">Be respectful and on-topic.</span>
          <button class="btn btn-primary" @click="createThread"
            :disabled="creating || !newThread.title || !newThread.content">
      ...
      <div v-else class="threads-list">
        <div v-for="t in threads" :key="t.threadID" class="thread-card card">
          <div class="card-body">
            <div class="post-header">
              <div class="user-badge">
                <div class="ua" :style="{ background: ac(t.threadTitle) }">
                  {{ (t.threadTitle || '?').slice(0, 2).toUpperCase() }}
                </div>
                <div>
                  <span class="uname">Thread #{{ t.threadID }}</span>
                </div>
              </div>
              <button class="vote-btn" @click="t.votes = (t.votes || 0) + 1">▲ {{ t.votes || 0 }}</button>
            </div>

            <h3 class="thread-title">{{ t.threadTitle }}</h3>
            <p class="thread-content">{{ t.content }}</p>

            <div class="thread-actions">
              <button class="action-btn" @click="toggleReply(t.threadID)">💬 Reply</button>
              <button class="action-btn" @click="toggleShowReplies(t)">
                {{ t.showReplies ? '▲ Hide replies' : '▼ Show replies' }}
              </button>
            </div>

            <div v-if="replyingTo === t.threadID" class="reply-box">
            ...
            <div v-if="t.showReplies && t.replies?.length" class="replies-wrap">
              <div v-for="r in t.replies" :key="r.threadID" class="reply-item">
                <div class="reply-avatar" :style="{ background: ac(r.content) }">R</div>
                <div class="reply-body">
                  <div class="reply-content">{{ r.content }}</div>
                </div>
              </div>
            </div>
            <div v-else-if="t.showReplies && !t.replies?.length" class="text-sm text-muted" style="margin-top:10px">
              No replies yet.
            </div>
          </div>
        </div>
...
const ac = str => COLORS[(str?.charCodeAt(0) || 0) % COLORS.length]
...
const toggleShowReplies = async t => {
  t.showReplies = !t.showReplies
  if (t.showReplies && !t.repliesLoaded) {
    try {
      const r = await forumService.getReplies(t.threadID)
      t.replies = r.data.replies || r.data
      t.repliesLoaded = true
    } catch { t.replies = [] }
  }
}
...
const createThread = async () => {
  creating.value = true
  try {
    await forumService.createThread(forumId.value, newThread.value)
    const tRes = await forumService.getThreads(forumId.value)
    threads.value = (tRes.data.threads || tRes.data).map(t => ({
      ...t, showReplies: false, replies: [], repliesLoaded: false, votes: 0
    }))
    newThread.value = { title: '', content: '' }
  } catch (e) {
    alert(e.response?.data?.error || 'Failed to post thread.')
  } finally { creating.value = false }
}

const postReply = async t => {
  try {
    await forumService.reply(t.threadID, { content: replyContent.value })
    const r = await forumService.getReplies(t.threadID)
    t.replies = r.data.replies || r.data
    t.repliesLoaded = true
    t.showReplies = true
...
onMounted(async () => {
  try {
    ...
    forum.value = list.find(f => f.forumID == forumId.value) || {}
    threads.value = (tRes.data.threads || tRes.data).map(t => ({
      ...t, showReplies: false, replies: [], repliesLoaded: false, votes: 0
    }))
  } catch {
    forum.value = { forumName: 'Forum' }
...
.replies-wrap { margin-top: 14px; padding-top: 14px; border-top: 1px solid var(--border); display: flex; flex-direction: column; gap: 10px; }
.reply-item { display: flex; gap: 10px; align-items: flex-start; }
.reply-avatar { width: 28px; height: 28px; border-radius: 50%; color: white; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.reply-body { flex: 1; background: var(--surface-2); border-radius: var(--radius); padding: 10px 14px; }
